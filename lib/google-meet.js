 ...
 */
async function createMeetLink({ summary, description, dateLabel, timeSlot, duration, attendees, startISO: exactStart, endISO: exactEnd, hideGuestList }) {
  const token = await accessToken();
  /* parseSlot infers the year from today, which is right for a booking label
     like "Sep 10" but wrong for anything dated further out. A caller holding a
     real timestamp passes it instead. */
  const slot = (exactStart && exactEnd) ? { startISO: exactStart, endISO: exactEnd } : parseSlot(dateLabel, timeSlot, duration);
  ...
  const resp = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      /* A webinar's guest list is every registrant, and Calendar shows that list
         to all of them. Off means nobody sees who else registered. Left alone
         for a 1:1 booking, where both parties are meant to see each other. */
      ...(hideGuestList ? { guestsCanSeeOtherGuests: false } : {}),
      conferenceData: {
        createRequest: {
...
/* Move an existing event rather than creating a second meeting when a webinar is
   rescheduled. Google keeps the same Meet link and notifies everyone already on
   the invite. */
async function updateMeetEvent({ eventId, dateLabel, timeSlot, duration, summary, description, startISO: exactStart, endISO: exactEnd }) {
  const id = String(eventId || '').trim();
  if (!id) throw Object.assign(new Error('An event id is required to update a meeting'), { status: 400 });
  const token = await accessToken();
  const slot = (exactStart && exactEnd) ? { startISO: exactStart, endISO: exactEnd } : parseSlot(dateLabel, timeSlot, duration);
  const { startISO, endISO } = slot;
  const body = {
  ...
  const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(googleMeetConfig().calendarId)}`
    + `/events/${encodeURIComponent(id)}?conferenceDataVersion=1&sendUpdates=all`;
  const resp = await fetch(url, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  if (!resp.ok) {
    const detail = (data.error && (data.error.message || data.error.status)) || `HTTP ${resp.status}`;
    throw Object.assign(new Error(`Google Calendar rejected the update: ${detail}`), { status: 502 });
  }
  return { link: eventMeetLink(data), eventId: data.id || id, startISO, endISO };
}

/* Add a registrant to the event so Google sends them the same calendar invite a
   1:1 booking produces. Existing guests are preserved - Calendar replaces the
   attendee list wholesale, so it is read first. */
async function addMeetAttendees({ eventId, emails, hideGuestList, notify }) {
  const id = String(eventId || '').trim();
  const wanted = (emails || [])
    .map((email) => String(email || '').trim().toLowerCase())
    .filter((email) => /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email));
  if (!id || !wanted.length) return { ok: false, skipped: true };
  const token = await accessToken();
  const base = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(googleMeetConfig().calendarId)}`
    + `/events/${encodeURIComponent(id)}`;
  const current = await fetch(base, { headers: { Authorization: `Bearer ${token}` } });
  const event = await current.json().catch(() => ({}));
  if (!current.ok) {
    const detail = (event.error && (event.error.message || event.error.status)) || `HTTP ${current.status}`;
    throw Object.assign(new Error(`Google Calendar could not read the event: ${detail}`), { status: 502 });
  }
  const existing = Array.isArray(event.attendees) ? event.attendees : [];
  const have = new Set(existing.map((a) => String(a && a.email || '').toLowerCase()));
  const added = wanted.filter((email) => !have.has(email));
  if (!added.length) return { ok: true, alreadyInvited: true, eventId: id };
  const attendees = existing.concat(added.map((email) => ({ email, responseStatus: 'needsAction' })));
  /* Calendar can only notify everyone or no one, so notifying on a webinar
     would mail every existing registrant each time somebody new signs up.
     notify:false leaves the new guest their calendar entry, and our own
     confirmation email is what carries them the link. */
  const send = notify === false ? 'none' : 'all';
  const resp = await fetch(`${base}?sendUpdates=${send}`, {
    method: 'PATCH',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(hideGuestList ? { attendees, guestsCanSeeOtherGuests: false } : { attendees }),
  });
  const data = await resp.json().catch(() => ({}));
  if (!resp.ok) {
    const detail = (data.error && (data.error.message || data.error.status)) || `HTTP ${resp.status}`;
    throw Object.assign(new Error(`Google Calendar rejected the invite: ${detail}`), { status: 502 });
  }
  return { ok: true, eventId: id, invited: added };
}
