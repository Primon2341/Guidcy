...
async function getAuthenticatedUser(req) {
  ...
  const anonKey = env('SUPABASE_ANON_KEY') || env('NEXT_PUBLIC_SUPABASE_ANON_KEY') || env('VITE_SUPABASE_ANON_KEY') || serviceKey;
  const bearer = clean(String(req && req.headers && req.headers.authorization || '').replace(/^Bearer\s+/i, ''), 3000);
  if (!bearer) throw Object.assign(new Error('Signed-in session is required'), { status: 401 });
  ...
  if (!response.ok || !user || !user.id) throw Object.assign(new Error('Signed-in session is invalid'), { status: 401 });
  return user;
}

async function canReconcileMarketplaceOrder(user, order) {
  if (!user || !order) return false;
  if (String(order.buyer_id || '') === String(user.id)) return true;
  if (clean(order.buyer_email, 320).toLowerCase() && clean(order.buyer_email, 320).toLowerCase() === clean(user.email, 320).toLowerCase()) return true;
  const profile = await first(`profiles?id=eq.${encodeURIComponent(user.id)}&select=role,email&limit=1`);
  const role = clean(profile && profile.role, 60).toLowerCase();
  const email = clean(user.email || profile && profile.email, 320).toLowerCase();
  const adminEmails = new Set([env('ADMIN_EMAIL'), env('ADMIN_EMAILS'), 'guidcytechnologies@gmail.com']
...
async function fetchRazorpayOrderPayments(orderId) {
  ...
  try { data = text ? JSON.parse(text) : null; } catch (_) { data = { raw: text }; }
  if (!response.ok) {
    throw Object.assign(new Error((data && (data.error && data.error.description || data.message)) || 'Unable to reconcile Razorpay order'), { status: response.status, data });
  }
  return Array.isArray(data && data.items) ? data.items : [];
}

async function createRazorpayRefund(paymentId, input = {}) {
  const cfg = getRazorpayConfig();
  const cleanPaymentId = clean(paymentId, 120);
  if (!/^pay_[A-Za-z0-9]+$/.test(cleanPaymentId)) {
    throw Object.assign(new Error('A valid Razorpay payment id is required for the refund'), { status: 400 });
  }
  const amount = Number(input.amount);
  if (!Number.isInteger(amount) || amount <= 0) {
    throw Object.assign(new Error('Refund amount must be a positive paise value'), { status: 400 });
  }
  const idempotencyKey = clean(input.idempotencyKey, 100);
  if (!idempotencyKey) {
    throw Object.assign(new Error('Refund idempotency key is required'), { status: 400 });
  }
  const body = { amount, speed: clean(input.speed || 'normal', 20) || 'normal' };
  if (input.notes && typeof input.notes === 'object') body.notes = input.notes;
  const response = await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(cleanPaymentId)}/refund`, {
    method: 'POST',
    headers: {
      Authorization: `Basic ${basicAuth(cfg.keyId, cfg.keySecret)}`,
      'Content-Type': 'application/json',
  ...
  let data = null;
  try { data = responseText ? JSON.parse(responseText) : null; } catch (_) { data = { raw: responseText }; }
  if (!response.ok) {
    const message = data && (data.error && (data.error.description || data.error.reason) || data.message);
    throw Object.assign(new Error(message || 'Unable to create Razorpay refund'), { status: response.status, data });
  }
  return data;
}
...
async function fetchRazorpayRefund(refundId) {
  const cfg = getRazorpayConfig();
  const cleanRefundId = clean(refundId, 120);
  if (!/^rfnd_[A-Za-z0-9]+$/.test(cleanRefundId)) {
    throw Object.assign(new Error('A valid Razorpay refund id is required'), { status: 400 });
  }
  const response = await fetch(`https://api.razorpay.com/v1/refunds/${encodeURIComponent(cleanRefundId)}`, {
    headers: { Authorization: `Basic ${basicAuth(cfg.keyId, cfg.keySecret)}` },
  });
  const responseText = await response.text();
  let data = null;
  ...
  if (!response.ok) {
    const message = data && (data.error && (data.error.description || data.error.reason) || data.message);
    throw Object.assign(new Error(message || 'Unable to fetch Razorpay refund'), { status: response.status, data });
  }
  return data;
}
