...
const { storedMeeting } = require('./webinar-meeting');
const { addMeetAttendees } = require('./google-meet');
...
async function sendWebinarRegistrationEmails(registration) {
  ...
  /* Put the registrant on the webinar's calendar event so Google sends them the
     same invitation a 1:1 booking produces, and carry the join link into the
     email. Both are best effort: a calendar hiccup must not cost them the
     confirmation. */
  let meetLink = '';
  try {
    const meeting = await storedMeeting(row.webinar_id);
    if (meeting && meeting.meet_link) {
      meetLink = meeting.meet_link;
      if (meeting.event_id) {
        await addMeetAttendees({ eventId: meeting.event_id, emails: [clean(row.email, 160)] });
      }
    }
  } catch (error) {
    console.warn('Webinar calendar invite skipped:', error && error.message);
  }
