  ...
  try {
    await supabaseRest('booking_session_logs', {
      ...
      body: {
        ...
        action_type: lifecycle === 'refunded' ? 'refund_processed' : lifecycle,
...
async function refundBookingRequest(req, body) {
  ...
  try {
    ...
    if (booking.refund_status === 'refunded') {
      return { statusCode: 200, data: { ok: true, refunded: true, idempotent: true, booking } };
    }
    ...
    const updated = await patchById('booking', booking.id, {
      ...
      payment_status: lifecycle === 'refunded' ? 'refunded' : 'success',
      payout_status: booking.payout_status === 'paid' ? 'paid' : 'not_required',
      updated_at: now,
    });
