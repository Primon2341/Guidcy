...
// llama-3.3-70b-versatile moved to Groq's Enterprise tier, so ordinary keys now
// get "does not exist or you do not have access to it". Override with GROQ_CHAT_MODEL.
...
function seedDocuments() {
  return [
    ...
    },
    {
      ...
      content: 'Users browse consultants, select an expert, choose a suitable session, complete payment, and then get access to the session details. Paid bookings use the Razorpay payment flow configured on the server. Booking status and payment status are updated after payment verification.'
    },
    {
    ...
    },
    {
      ...
      content: 'Guidcy webinars can be free or paid. Users register from the webinars page. Paid webinar registration uses the same server-side Razorpay flow as other paid actions, and successful payment confirms the registration.'
    },
    {
      source_type: 'policy',
