...
function strictRelevanceScore(item, query, kind = 'generic') {
  const terms = tokenize(query);
  if (!terms.length) return 100;
  const fieldMap = {
    job: [[item.title,50],[item.skills,45],[item.category,40],[item.company,25],[item.location,20],[item.employmentType || item.work_type,20],[item.description,15]],
    education: [[item.program || item.course || item.title,50],[item.field || item.stream || item.category,40],[item.institution || item.college || item.university,25],[item.eligibility || item.description || item.whyFit,15]],
    generic: [[item.title || item.name,45],[item.category || item.stream,40],[item.skills || item.tags,35],[item.description || item.summary || item.whyFit,15]]
  };
  const fields = fieldMap[kind] || fieldMap.generic;
  let score = 0;
  for (const [value, weight] of fields) {
    const text = String(Array.isArray(value) ? value.join(' ') : (value || '')).toLowerCase();
    const words = tokenize(text);
    for (const term of terms) {
      if (words.includes(term)) score += Math.round(weight * 0.9);
      else if (text.includes(term)) score += Math.round(weight * 0.55);
    }
  }
  if (fields.map(([v]) => String(v || '').toLowerCase()).join(' ').includes(String(query || '').toLowerCase())) score += 20;
  return Math.min(100, score);
}
function strictFilter(items, query, kind, threshold = 40) {
  return (items || [])
    .map((item) => ({ ...item, score: item.score || strictRelevanceScore(item, query, kind), relevanceScore: strictRelevanceScore(item, query, kind) }))
    .filter((item) => item.relevanceScore >= threshold || Number(item.score || 0) >= threshold)
...
module.exports = async function handler(req, res) {
  ...
  if (mode === 'jobs') {
    ...
    let rankedJobs = allJobs.slice(0, 20), aiSummary = '', topTips = [];

    if (allJobs.length > 0) {
      try {
        const raw = await callGroq(
          'You are a career counsellor. Return ONLY valid JSON. No markdown, no explanation. Be strict: do not recommend unrelated courses, colleges, or careers just to fill the list.',
...
JOBS:
${allJobs.slice(0, 30).map((j, i) => `[${i}] ${j.title} at ${j.company} (${j.location}): ${j.description.slice(0, 150)}`).join('\n')}
...
Include the top 15-20 jobs by score when enough relevant live jobs are available. Do not include any job with weak or indirect relevance. A job is valid only when title, skills, category, description, company, or location clearly matches the candidate inputs.`,
          2600
        );
        const parsed = parseJSON(raw);
      ...
      try {
        const raw = await callGroq(
          ...
          `Generate 18 realistic job recommendations for this candidate. Return ONLY a JSON array:\n[{"title":"...","company":"...","location":"...","employmentType":"Full-time","description":"...","applyLink":"https://linkedin.com/jobs","salary":"","score":85,"whyFit":"2 sentences specific to this candidate"}]\n\n${profileText}`,
          1200
        );
        rankedJobs = []; // Do not hallucinate unavailable jobs when the live source has no relevant openings.
      } catch (_) {}
      try {
        const raw = await callGroq(
    ...
    const strictQuery = [p.currentRole, p.skills, p.industry].filter(Boolean).join(' ') || queries.join(' ');
    rankedJobs = strictFilter(rankedJobs, strictQuery, 'job', 40).slice(0, 20);
    return res.status(200).end(JSON.stringify({
      mode: 'jobs', queries, totalFound: rankedJobs.length,
      jobs: rankedJobs, summary: rankedJobs.length ? aiSummary : 'No highly relevant jobs found for this profile. Try broader skills, role, or location inputs.', tips: topTips
    }));
  }
  ...
  if (mode === 'education') {
    ...
    try {
      const raw = await callGroq(
        'You are an expert academic counsellor with deep knowledge of Indian and international universities, programs, eligibility criteria, fees, and admission processes. Always use real institution names and accurate information. Return ONLY valid JSON. No markdown, no explanation. Be strict: do not recommend unrelated courses, colleges, or careers just to fill the list.',
...
Include 15-20 programs mixing Safe Pick, Good Fit, and Stretch Goal categories when enough strong options exist. Be specific with real universities and accurate requirements.`,
        5000
      );
