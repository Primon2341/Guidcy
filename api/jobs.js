const opportunitiesHandler = require('../lib/opportunities-handler');
...
function relevanceScore(job, query) {
  ...
  const fields = [
    ...
    [job.company, 25],
    [job.location, 20],
    [job.employmentType || job.work_type, 20],
    [job.description, 15]
  ];
  let score = 0;
...
module.exports = async function handler(req, res) {
  if (String(req.query && req.query.service || '').toLowerCase() === 'opportunities') {
  ...
  try {
    ...
    const rankedJobs = jobs
      .map((job) => ({ ...job, relevanceScore: relevanceScore(job, q) }))
      .filter((job) => job.relevanceScore >= 40)
    ...
    return res.status(200).end(JSON.stringify({
      ...
      message: rankedJobs.length ? undefined : 'No highly relevant jobs found for this search. Try a broader keyword.'
    }));
  } catch (error) {
